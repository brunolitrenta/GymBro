    ...
    function renderExercise({ item }: { item: IExercise }) {
        ...
        return (
            <Link asChild href={{
                ...
                params: { exercise: JSON.stringify(item.id) }
            }}>
    ...
    return (
        <SafeAreaView edges={['top']} className="flex-1 flex-column justify-evenly items-center">
            ...
            <View className='w-full h-5/6'>
                <Text className='text-darkgreen font-rbold ml-2 text-base ml-5'>Lista de exercícios</Text>
            ...
            <TouchableOpacity disabled={exercisesCompleted.length == 0 ? true : false} activeOpacity={0.7}
                className={
